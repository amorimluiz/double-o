...
| `step` | `prd` \| `techspec` \| `design` \| `tasks` \| `workspace` \| `execute` \| `qa-report` \| `qa-execution` \| `review` \| `done` \| `blocked` | Current stage, in this order. Extensible: further steps plug in between `execute` and `done`. |
| `iteration` | int ≥ 0 | Completed execution iterations. |
| `created_at` / `updated_at` | RFC3339 UTC | First write / last write. |
| `artifacts.*` | `pending` \| `done` \| `skipped` | Artifact status. `skipped` is valid only for `design`, `qa-report`, and `qa-execution`, and only on a recorded applicability decision. |
...
artifacts: {prd: done, techspec: done, design: done, tasks: done, qa-report: done, qa-execution: pending, review: pending}
...
| `00-design` | `artifacts.design`, `step`, `updated_at` |
...
5. `done` is the only successful completion; it requires every task completed and verified, the design artifact `done` or `skipped`, both QA artifacts `done` or `skipped`, and the review artifact `done`.
6. `skipped` QA requires the applicability check: the executed change added no interface a user or caller exercises (UI, HTTP/UDS API, CLI, SDK). The reason is recorded in `next` at the skip. `skipped` design requires the same kind of recorded decision: the feature added no surface a user sees.
