---
name: 00-loop
description: Run the full spec-driven development loop for a feature — generate PRD, TechSpec, and tasks behind approval gates, execute tasks one per iteration with TDD, verification, and atomic commits, then run the applicable QA steps and a deep review, resuming from state.yml. Use when the user asks to build a feature end-to-end from a prompt, or to resume or continue an SDD run. Do not use for one-off edits or single tasks without a spec.
---
...
- Skip a step whose artifact is `done` or `skipped`; jump straight to the next pending one.
...
- Every task completed and verified: advance to `qa-report` (or straight to `review` on a recorded QA skip) and keep the pipeline moving. The run reaches `done` only after the QA steps and the review gate pass; then report the summary — tasks executed, commits, verification status, anything left over.
